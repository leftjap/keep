...
- 댓글창이 화면 하단에 고정되지 않던 문제 수정 — position:sticky bottom:0 적용 + 댓글 표시 시 editor-content-wrap padding-bottom 제거. [UI.댓글레이아웃] (style.css, js/ui.js)
...
- 새 글 작성 시 입력 내용이 1~2초 후 사라지는 버그 수정 — handleNew 직후 mergeServerAll이 서버 데이터로 로컬을 교체하여 새 문서 소실. _editorDirty는 doSaveAndSync에서 즉시 리셋되어 보호 불충분. _unsyncedLocal 설정으로 saveDatabase 성공까지 서버 교체 차단. [상태.플래그리셋] (js/ui.js)
