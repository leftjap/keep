...
- 메인 프로젝트: C:\Users\leftj\Documents\바이브 코딩\nametag\
- GAS 프로젝트: C:\Users\leftj\Documents\바이브 코딩\nametag\gas\
...
### Step 1
- 파일: [절대 경로] (예: C:\Users\leftj\Documents\바이브 코딩\nametag\js\ui.js)
...
- 모든 Step의 `파일:` 항목에 절대 경로를 사용한다. 상대 경로(`js/ui.js`)가 아닌 전체 경로(`C:\Users\leftj\Documents\바이브 코딩\nametag\js\ui.js`)를 명시한다. Haiku가 프로젝트 위치를 찾지 못하는 것을 방지한다.
...
- 작업:
  ```bash
  cd "C:\Users\leftj\Documents\바이브 코딩\nametag"
  git add .
  git commit -m "[타입]: [요약]"
...
```
### Step N: GAS 배포
- 파일: C:\Users\leftj\Documents\바이브 코딩\nametag\gas\Code.js
- 작업:
  ```bash
  cd "C:\Users\leftj\Documents\바이브 코딩\nametag\gas"
  clasp push
...
**경로:** 메인 레포(`nametag`) 내 하위 폴더. `~/바이브 코딩/nametag/gas/`
...
- Code.gs는 메인 레포의 `gas/` 폴더에 있으므로 Git 관리 대상이다. Code.gs 변경 시 `clasp push`로 GAS에 배포하고, Git 커밋에도 포함한다.
...
**갱신 시점과 대상:** 2번 "WORKFLOW.md 갱신 규칙"을 따른다.
...
a. WORKFLOW.md 상세 맵(8번)에서 관련 함수/전역 변수를 특정한다.
...
- `gas/Code.gs` (~1,150줄) — 크롤러가 잘라서 반환할 확률이 있다. GAS 수정이 필요한 경우 업로드 요청.
...
- 압축 후에는 WORKFLOW.md의 상세 맵(8번)에 있는 함수명/변수명을
...
| SMS 가계부 자동 반영 안 됨 | sync.js (mergeServerExpenses) + gas/Code.gs (saveExpenseFromSMS) |
| GAS 동기화 실패 | sync.js (SYNC._post) + gas/Code.gs (doPost) |
...
| 매출처명 정제 안 됨 | data.js (cleanMerchantName) + gas/Code.gs (cleanMerchantName) |
...
| 날짜 | 변경 내용 |
|---|---|
| 2026-03-14 | data.js에 BRAND_CATEGORY_MAP(143개) + cleanMerchantName + getCategoryByBrand 추가. newExpense/updateExpense에 자동 정제·분류. ui-expense.js에 validateIconUrl 추가, _renderYearlyBubbles 아이콘 없는 브랜드→카테고리 묶음, openCategoryExpensePopup 연간 모드 브랜드 제외. Code.gs에 동일 상수·함수 추가, saveExpenseFromSMS/importCardSmsSheet에 적용 |
