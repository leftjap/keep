...
3. AI는 상세 맵(8번)에서 관련 파일/함수를 특정한 뒤,
   업로드된 파일이 부족하면 GitHub raw URL로 직접 크롤링한다.
   크롤링 불가 조건(23번 "소스 참조 프로토콜" 참조)에 해당하면
   사용자에게 추가 파일을 요청한다.
...
- 모든 Step의 `파일:` 항목에 절대 경로를 사용한다. 상대 경로(`js/ui.js`)가 아닌 전체 경로(`C:\dev\nametag\js\ui.js`)를 명시한다. Haiku가 프로젝트 위치를 찾지 못하는 것을 방지한다.
- 커밋 & 푸시, clasp push 등 터미널 명령의 `cd` 경로도 절대 경로를 사용한다.
- gesture.js는 어떤 Step에서도 포함하지 않는다.

### AGENTS.md 갱신 규칙

이 문서의 목적은 AI가 현재 코드 구조를 정확히 파악하는 것이다. 작업 이력 기록이 아니다.

**갱신 대상 (코드 구조 정보만):**
- 8번(파일별 상세 맵): 함수/상수 추가·삭제·이름 변경 시 **반드시** 갱신
- 12번(전역 상태 변수): 추가·삭제 시 갱신
- 14번(핵심 함수 호출 체인): 흐름 변경 시 갱신
- 15번(데이터 스키마): 필드 변경 시 갱신
- 7번(파일 구조): 새 파일 추가 시 갱신

**갱신하지 않는 것:**
- 변경 로그 (git log로 대체)
- CSS만 변경한 경우 (구조 변경 아님)
- 함수 내부 로직만 수정하고 인터페이스가 동일한 경우

**AI 필수 체크 — 작업지시서 코드 Step을 모두 작성한 뒤, 아래를 확인한다:**
1. 함수가 추가/삭제/이름 변경되었는가? → 8번 갱신 Step 포함
2. 전역 변수/상수가 추가/삭제되었는가? → 8번, 12번 갱신 Step 포함
3. 데이터 스키마가 변경되었는가? → 15번 갱신 Step 포함
4. 호출 체인이 바뀌었는가? → 14번 갱신 Step 포함
5. 새 파일이 추가되었는가? → 7번 갱신 Step 포함

**위 5개를 확인하지 않고 작업지시서를 출력하지 않는다.** 하나라도 해당하면 AGENTS.md 갱신 Step을 반드시 포함한다. 모두 해당하지 않으면 갱신 Step을 생략한다.

### 커밋 & 푸시 규칙

모든 작업지시서의 마지막 Step에 커밋+푸시를 포함한다. AGENTS.md 갱신 Step이 있으면 그 이후에, 없으면 코드 변경 Step 이후에 배치한다.
...
- 갱신 후 별도 커밋:
  ...
  git push origin main
  ```
- playbook.md 크롤링 경로: `https://raw.githubusercontent.com/leftjap/playbook-config/main/playbook.md`
  사용자가 playbook.md를 업로드하지 않은 경우, 이 URL에서 현재 상태를 크롤링한 뒤 정확한 갱신 Step을 작성한다.
...
1. Code.gs 수정 Step(들)
2. `clasp push` Step
3. Git 커밋 & 푸시 Step (gas 폴더는 메인 레포에 포함되므로 항상 커밋)
4. (해당 시) AGENTS.md 갱신 Step

**Code.gs만 수정하고 메인 레포의 다른 파일은 변경하지 않은 경우**, gas 폴더의 변경도 Git에 커밋한다 (gas 폴더는 메인 레포 내에 있으므로 Git 관리 대상이다).
...
### GAS (프로젝트 내부 — clasp 배포)
gas/                   — Google Apps Script (메인 레포 내 하위 폴더)
  .clasp.json         — clasp 프로젝트 설정 (scriptId)
  appsscript.json     — GAS 런타임 설정 (V8, webapp)
  Code.js             — 서버 동기화, SMS 파싱, DB/문서/이미지/루틴/어구 저장
```
...
- `showExpConfirm(message, onResult)` — 커스텀 삭제 확인 모달 표시. onResult(true/false) 콜백
- `hideExpConfirm(result)` — 모달 닫기 + 콜백 실행
...
- `_unsyncedLocal` — hidden 시 syncAllSafe 실패로 서버에 못 올린 로컬 변경이 있는지 (visible 복귀 시 병합 판단용)
...
- `autoMatchCategory(merchant)` — 가맹점명 → 카테고리 자동 매칭 (12개 카테고리: food, dining, shopping, transport, subscribe, medical, leisure, beauty, pet, invest, utility, etc). Code.js의 `autoMatchCategoryServer()`와 동일 규칙 유지 필수
...
- `syncAll()` — 전체 동기화
- `syncAllSafe()` — hidden 시 서버 문서 updated 비교 후 _unsyncedLocal 플래그 설정. 서버가 더 최신이면 푸시 건너뜀, 실패하면 다음 visible에 로컬 보호
- `mergeServerDocs(dbData)` — 서버 docs/books/memos/quotes를 updated 기준으로 로컬에 병합. dbData 인자가 있으면 사용, 없으면 자체 load_db. _unsyncedLocal이 true면 병합 건너뜀
...
**경로:** 메인 레포(`nametag`) 내 하위 폴더. `C:\dev\nametag\gas\`
...
- `postComment(docId, docOwner, text, config, source)` — 댓글 저장 + 글 작성자에게 알림. source='claude'면 author='claude@ai', 아니면 현재 사용자 이메일. LockService 사용
...
hidden: saveLocalOnly → _editorDirty=false → syncAllSafe() → 서버 문서 updated 비교 → 서버가 더 최신이면 푸시 건너뜀, 실패하면 _unsyncedLocal=true 설정
visible (30초 쿨다운):
  _unsyncedLocal이면: syncAll() → _unsyncedLocal=false → mergeServerAll()
  _editorDirty가 false이면: mergeServerAll() (load_db 1회로 expenses + docs 병합)
  _editorDirty가 true이면: mergeServerExpenses()만 (문서 병합 안 함)
...
- Code.js의 `parseSMSServer()`와 클라이언트 `sms-parser.js`의 `parseSMS()`는 **같은 로직의 서버/클라이언트 버전**이다. 한쪽을 수정하면 다른 쪽도 동일하게 수정해야 한다. 작업지시서에 양쪽 수정 Step을 모두 포함한다.
- Code.js의 `autoMatchCategoryServer()`와 `sms-parser.js`의 `autoMatchCategory()`도 마찬가지다.
...
- Code.js는 메인 레포의 `gas/` 폴더에 있으므로 Git 관리 대상이다. Code.js 변경 시 `clasp push`로 GAS에 배포하고, Git 커밋에도 포함한다.
- `clasp push`만으로는 GAS 웹앱 URL에 최신 코드가 반영되지 않는다. **Code.js를 수정할 때마다** GAS 편집기에서 웹앱 재배포(배포 > 배포 관리 > 연필 아이콘 > 버전: 새 버전 > 배포)를 실행해야 한다. 이것은 사용자가 수동으로 수행하며, 작업지시서의 `clasp push` Step에 항상 안내를 포함한다.
...
| _expConfirmCallback | ui-expense.js | 삭제 확인 모달 콜백 함수 |
...
→ action 분기:
  ...
  ├─ 'post_comment': postComment(docId, docOwner, text, config, source) → saveSocialData() + 알림 추가 (source='claude'면 author='claude@ai')
...
```
{
  ...
  "comments": [
    { "id": "cmt_...", "docId": "...", "docOwner": "email", "author": "email", "text": "...", "created": "ISO", "source": "human"|"claude" }
  ]
}
```
...
이 문서는 AI가 코드 구조를 파악하기 위한 네비게이션 맵이다. 코드와 문서가 어긋나면 AI가 잘못된 판단을 한다.

**핵심 원칙:** 8번(파일별 상세 맵)이 항상 최신이면 AI는 구조를 정확히 파악할 수 있다.

**갱신 시점과 대상:** 2번 "AGENTS.md 갱신 규칙"을 따른다.
...
a. AGENTS.md 상세 맵(8번)에서 관련 함수/전역 변수를 특정한다.
b. 업로드된 소스 또는 GitHub raw URL에서 해당 함수의 로직을 추적한다.
c. 호출 체인(14번)과 전역 상태(12번)로 데이터 흐름을 시뮬레이션한다.
d. 가설을 세우고, 그 가설이 맞다면 어떤 코드를 수정해야 하는지까지 특정한다.
e. 배포 URL(https://leftjap.github.io/nametag-game/)로 접속 상태를 확인한다 (200 vs 404).
...
console.log('currentListView:', currentListView);
console.log('currentLoadedDoc:', currentLoadedDoc);
```

#### 가계부 폼 상태
```javascript
console.log('curExpenseId:', curExpenseId);
console.log('_expenseViewYM:', _expenseViewYM);
console.log('_smsPasteMode:', _smsPasteMode);
console.log('_expenseCategoryFilter:', _expenseCategoryFilter);
```

#### 가계부 데이터 확인
```javascript
JSON.parse(localStorage.getItem('nt_expenses')).length
JSON.parse(localStorage.getItem('nt_expenses')).filter(e => e.date.startsWith('2026-03'))
```

#### 가계부 월별 통계 확인
```javascript
getMonthExpenses(getExpenseViewYM())
```

#### 루틴 체크 확인
```javascript
JSON.parse(localStorage.getItem('nt_checks'))
```

#### 브랜드 아이콘 확인
```javascript
JSON.parse(localStorage.getItem('nt_brandIcons'))
```

#### 브랜드 오버라이드 확인
```javascript
JSON.parse(localStorage.getItem('nt_brandOverrides'))
```
...
#### DOM 상태 확인
```javascript
document.querySelector('.list-panel').classList.toString()
document.getElementById('expenseFullDashboard').style.display
document.getElementById('pane-expense-dashboard').style.display
```

#### 에디터 패널 표시 상태 (한번에 전체 확인)
```javascript
['editorText','editorBook','editorQuote','editorMemo','editorExpense','editorDayList','editorRoutineDetail','expenseFullDashboard'].forEach(id => {
  var el = document.getElementById(id);
  if(el && el.style.display !== 'none') console.log(id, ':', el.style.display);
});
```

### 디버깅 절차
1. 문제 재현 → 텍스트 설명 우선, 불충분하면 스크린샷 요청
...
## 24. 소스 참조 및 사용자 개입 최소화 프로토콜

### 배포 URL 및 소스 참조 경로

| 항목 | 값 |
|---|---|
| 배포 URL | `https://leftjap.github.io/nametag-game/` |
| GitHub raw base | `https://raw.githubusercontent.com/leftjap/nametag-game/main/` |
...
2. 상세 맵(8번) + 호출 체인(14번)에서 관련 파일/함수를 특정한다
...
사용할 것:
  `raw.githubusercontent.com/leftjap/nametag-game/main/{파일경로}`
...
- `style.css` (~3,000줄) — 크롤러가 잘라서 반환할 확률이 높다. 항상 업로드 요청.
- `gas/Code.js` (~1,150줄) — 크롤러가 잘라서 반환할 확률이 있다. GAS 수정이 필요한 경우 업로드 요청.
...
- 사용자가 업로드한 파일이 직전 작업지시서의 변경을 포함하는지 불확실하면,
  "이 파일은 최신 버전인가요?"라고 1회 확인한다.
...
- 요청 시 "확실히 필요한 파일"과 "높은 확률로 필요한 파일"을 구분해서 안내한다
  예: "ui-expense.js, style.css 업로드 필요. data.js도 관련될 수 있으니 함께 올려주시면 좋습니다"
...
- 압축 후에는 AGENTS.md의 상세 맵(8번)에 있는 함수명/변수명을
...
| 증상 | 의심 파일 |
|---|---|
| 탭 전환 후 화면 깨짐 | ui.js (switchTab) — 에디터 패널 복원 누락 가능성 |
| 가계부 대시보드 빈 화면 | ui-expense.js (renderExpenseDashboard) + data.js (getMonthExpenses) |
| 가계부 차트/타임라인 비정상 | ui-expense.js (renderCumulativeChart, renderRecentExpenses) |
| 가계부 폼 저장 안 됨 | ui-expense.js (saveExpenseForm) + editor.js |
| 가계부 뷰 스위처 노출됨 | style.css (.expense-active) + ui.js (switchTab) |
| 루틴 체크 안 됨 | routine.js (toggleChk, toggleDay) |
| 루틴 캘린더 렌더 안 됨 | routine-cal.js (renderRoutineCalView) |
| 에디터 자동저장 안 됨 | editor.js (setupAutoSave) |
| 리스트 렌더링 비정상 | ui.js (renderListPanel, generateItemHtml) |
| 사이드바 글 개수 안 보임 | ui.js (renderWritingGrid) + data.js (getTabCount) |
| SMS 가계부 자동 반영 안 됨 | sync.js (mergeServerExpenses) + gas/Code.js (saveExpenseFromSMS) |
| GAS 동기화 실패 | sync.js (SYNC._post) + gas/Code.js (doPost) |
| 브랜드 아이콘 안 나옴 | ui-expense.js (getMerchantIconHtml) + data.js (getBrandIcon) |
| 연간 버블/랭킹 비정상 | ui-expense.js (_renderYearlyBubbles, _renderYearlyRankList) |
| 매출처명 정제 안 됨 | data.js (cleanMerchantName) + gas/Code.js (cleanMerchantName) |

---

## 변경 로그 (최근 5개, 이전 이력은 git log 참조)

| 날짜 | 변경 내용 |
|---|---|
| 2026-03-14 | data.js에 BRAND_CATEGORY_MAP(143개) + cleanMerchantName + getCategoryByBrand 추가. newExpense/updateExpense에 자동 정제·분류. ui-expense.js에 validateIconUrl 추가, _renderYearlyBubbles 아이콘 없는 브랜드→카테고리 묶음, openCategoryExpensePopup 연간 모드 브랜드 제외. Code.js에 동일 상수·함수 추가, saveExpenseFromSMS/importCardSmsSheet에 적용 |
| 2026-03-14 | 22번 매출처명 정제 + 브랜드 카테고리 자동 분류 시스템 섹션 추가 |
| 2026-03-14 | 브랜드 시스템 4단계: 별명 비활성화, 가계부 폼 재구성, 21번 가계부 데이터 운영 규칙 추가 |
| 2026-03-13 | 브랜드 아이콘 마스터 공유: Code.js loadDatabase에서 leftjap의 brandIcons를 항상 마스터로 읽어서 masterBrandIcons로 응답. sync.js loadDatabase에서 masterBrandIcons를 LocalStorage brandIcons와 병합 |
| 2026-03-13 | 연간 누적 섹션: 버블 차트 + 랭킹 리스트 + 카테고리 트리맵 + 월별 집계(endYM) |
...
| 2026-03-11 | 별명 매출처 아이콘 매칭 실패 수정: reverseAlias()(data.js) 추가하여 별명→원본 역조회, findMerchantIcon()에서 직접 매칭 실패 시 역조회 후 재검색, AGENTS.md 8번/ui-expense.js 폼 관리 설명 갱신 |
...
| 2026-03-12 | 아내 가계부 카드 매핑 보강: soyoun312 cardNameMap에 신한8619 추가, parseSMSServer/parseSMS에 현대백화점카드 태그/지점명/신한체크 (금액) 태그 정제 추가, AGENTS.md 10번에 사용자별 카드 현황 표 추가 |
| 2026-03-13 | 연간 "그 외" 태그/기타 팝업 반영: AGENTS.md 8번에 _renderEtcBanner, openEtcGroupPopup, openCategoryEtcPopup 추가, _renderYearlyBubbles 설명 갱신(isEtcGroup), getYearMerchantBreakdown 설명 갱신(isEtcGroup) |
