...
#### ⛔ Freeze: lpPopup ↔ expFloatingPopup z-index 관계
- 불변 조건:
  - `lpPopupOverlay`(z-index 9900)는 `expFloatingPopupOverlay`(z-index 9800)보다 반드시 **위**에 위치해야 함. 역전 시 수정/삭제 메뉴의 dismiss가 불가능해짐.
  - `lpPopupCard`(z-index 10000)는 모든 overlay보다 위에 위치해야 함.
  - `showExpensePopup`에서 플로팅 팝업이 open일 때 `lpPopupOverlay`를 `background:transparent` + `backdrop-filter:none`으로 설정해야 함. 제거 시 플로팅 팝업이 가려짐.
  - `closeLpPopup`의 clone 경로(100ms setTimeout)와 non-clone 경로 모두에서 `overlay.style.background = ''` 등 인라인 스타일 초기화가 존재해야 함. 제거 시 후속 팝업(toggleEditorMenu 등)의 overlay가 투명으로 열림.
- 소스 주석: `showExpensePopup`의 조건부 투명 설정 블록, `closeLpPopup`의 인라인 스타일 초기화 블록 (⛔ FREEZE 태그)
- 테스트: 수동 — ①플로팅 팝업 위 꾹누르기 → 메뉴 + 플로팅 팝업 공존 ②바깥 클릭 → 메뉴 dismiss ③글쓰기 리스트 꾹누르기 → 반투명 blur 정상

#### 🛡️ Guard: server-wins 동기화 불변 조건
- 불변 조건:
  - 서버 응답 수신 시 로컬을 서버 데이터로 교체함 (서버가 source of truth).
  - 서버의 `_deleted` 항목은 로컬 localStorage에 보존함 (`saveDatabase`의 `_deletedIds` 구성에 필요). UI에서는 `_filterDeleted()`로 숨김.
  - `_unsyncedLocal === true`이면 서버 교체를 건너뜀.
  - `_dbSaveQueued === true`이면 `mergeServerAll`을 건너뜀 (race condition 방지).
  - 급감 가드: 로컬 항목 > 0이고 서버 항목 = 0이면 교체를 차단함.
- 테스트: `__tests__/merge-integrity.test.js` (`npm test`로 자동 실행) — server-wins 기준으로 갱신 필요
